...
export async function POST(req: Request) {
  try {
    ...
    const user = { id: `user_${username}`, username, email: `${username}@example.com`, roles };
    await upsertUser({ id: user.id, email: user.email, name: username });
    const token = jwt.sign({ sub: user.id, username, roles, tenant_id }, secret, { expiresIn: '1h' });
    const res = NextResponse.json({ token, user });
    res.cookies.set('session_token', token, { httpOnly: true, sameSite: 'lax', secure: false, maxAge: 3600 });
    return res;
  } catch (err) {
