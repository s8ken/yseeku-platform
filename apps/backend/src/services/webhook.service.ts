...
import { WebhookConfigModel, IWebhookConfig, AlertRule, AlertRuleCondition } from '../models/webhook-config.model';
...
class WebhookService {
  ...
  private async checkRateLimit(config: IWebhookConfig): Promise<boolean> {
    ...
    const { maxPerMinute = 60, maxPerHour = 1000 } = config.rateLimiting;
