...
<head>
  <style>
    ...
    .severity { display: inline-block; padding: 4px 12px; background: ${color}; color: white; border-radius: 4px; }
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>This alert was generated by Yseeku Platform</p>
    </div>
  </div>
</body>
</html>
  ...
  // Access channels safely throughout the file
  private getChannelUrl(config: IWebhookConfig): string | undefined {
    const channels = config.channels ?? [];
    const firstChannel = channels[0];
    if (!firstChannel) return undefined;
    if (typeof firstChannel === 'string') return undefined;
    return (firstChannel as WebhookChannelConfig).url;
  }

  private getChannelMethod(config: IWebhookConfig): string {
    const channels = config.channels ?? [];
    const firstChannel = channels[0];
    if (!firstChannel || typeof firstChannel === 'string') return 'POST';
    return (firstChannel as WebhookChannelConfig).method ?? 'POST';
  }

  private getTimeoutMs(config: IWebhookConfig): number {
    const retryConfig = config.retryConfig as RetryConfig | undefined;
    return retryConfig?.timeoutMs ?? 30000;
  }

  // Line ~122: Fix channels access
  async validateConfig(config: IWebhookConfig): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = [];
    const channels = config.channels ?? [];
    ...
    for (const channel of channels) {
      if (typeof channel === 'string') {
        // Simple string channel, no further validation
        continue;
      }
      
      // Object channel, validate fields
      if (!channel.url) {
  ...
  // Line ~142: Fix conditions access
  private evaluateConditions(rule: AlertRule, eventData: Record<string, unknown>): boolean {
    const conditions = rule.conditions ?? [];
    if (conditions.length === 0) return true;
    
    for (const condition of conditions) {
      const fieldValue = eventData[condition.field];
      if (fieldValue === undefined) continue; // Skip if field not in event data
  ...
  // Line ~176: Fix rateLimiting access
  private checkRateLimit(config: IWebhookConfig): boolean {
    const rateLimiting = config.rateLimiting;
    if (!rateLimiting || !(rateLimiting as { enabled?: boolean }).enabled) {
      return true;
