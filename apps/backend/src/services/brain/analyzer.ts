...
/**
 * Analysis result from the Overseer's context analysis
 */
 ...
 */
export function analyzeContext(input: SensorData): AnalysisResult {
 ...
 */
export function analyzeContextLegacy(input: { bedau: any; avgTrust: number; receipts: any[] }) {
  const observations: string[] = [];
  const trust = input.avgTrust;
  const emergence = input.bedau?.emergence_type || 'LINEAR';
