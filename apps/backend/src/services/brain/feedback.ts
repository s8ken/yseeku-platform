...
import { remember, recall, recallMany } from './memory';
...
export interface ActionOutcome {
  actionId: string;
  success: boolean;
  impact: number; // -1 to 1 scale (-1 = very negative, 0 = neutral, 1 = very positive)
  metrics?: {
    trustDelta?: number;
...
/**
 * Effectiveness score for an action type
 */
...
/**
 * System state for measuring impact
 */
...
/**
 * Action recommendation from the feedback loop
 */
...
export async function recordActionOutcome(
  ...
  const action = await BrainAction.findById(outcome.actionId);
  if (!action) {
    logger.warn('Action not found for outcome recording', { actionId: outcome.actionId });
    return;
  }
...
export async function getActionRecommendations(
  ...
  for (const actionType of actionTypes) {
    ...
    const cachedEffectiveness = await recall(tenantId, `effectiveness:${actionType}`);

    let effectiveness: EffectivenessScore;
    if (cachedEffectiveness && cachedEffectiveness.payload.sampleSize >= 5) {
      effectiveness = cachedEffectiveness.payload as EffectivenessScore;
    } else {
 ...
 */
export async function measureActionImpact(
  tenantId: string,
  actionId: string,
  preActionState: SystemState,
  postActionState: SystemState
...
export async function getLatestRecommendations(
  tenantId: string
): Promise<{ adjustments: ActionRecommendation[] } | null> {
  const memory = await recall(tenantId, 'feedback:recommendations');
  if (!memory) return null;
  return memory.payload as { adjustments: ActionRecommendation[] };
}

/**
 * Analyze trends in action effectiveness over time
 */
export async function analyzeEffectivenessTrends(
  tenantId: string,
  actionType: string,
  windowDays: number = 90
): Promise<{
  trend: 'improving' | 'declining' | 'stable';
  dataPoints: Array<{ date: string; successRate: number; avgImpact: number }>;
}> {
  const windowStart = new Date();
  windowStart.setDate(windowStart.getDate() - windowDays);
  ...
  const weeklyData: Map<string, { success: number; total: number; impact: number }> = new Map();

  for (const action of actions) {
    const weekKey = getWeekKey(action.executedAt);
    const existing = weeklyData.get(weekKey) || { success: 0, total: 0, impact: 0 };
    existing.total++;
    if (action.result?.outcome?.success) existing.success++;
