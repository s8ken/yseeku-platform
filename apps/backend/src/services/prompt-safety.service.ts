...
interface HeuristicCheck {
  name: string;
  check: (text: string) => { triggered: boolean; confidence: number; details?: string };
  category: ThreatCategory;
  severity: ThreatLevel;
  mitigation: string;
}
...
class PromptSafetyScanner {
  ...
  private heuristics: HeuristicCheck[];
   ...
   */
  sanitize(text: string): { sanitized: string; removed: string[] } {
    const result = this.scan(text);
    let sanitized = text;
  ...
  private getSnippet(text: string, start: number, length: number): string {
...
export const promptSafetyScanner = new PromptSafetyScanner();

// Export types and class
export { PromptSafetyScanner };
