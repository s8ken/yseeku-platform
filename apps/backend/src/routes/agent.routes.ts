...
router.get('/', protect, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const agents = await Agent.find(query).select('-apiKeyId').sort({ lastActive: -1 });
    ...
    const enhancedAgents = agents.map((agent) => {
      ...
      if (!metrics || metrics.messageCount === 0) {
        return {
          ...
          trustScore: 85,
          interactionCount: 0,
          lastInteraction: agent.lastActive?.toISOString(),
          sonateDimensions: {
            realityIndex: 8.5,
            trustProtocol: 'PASS',
      ...
      const totalStatuses =
        metrics.statuses.pass + metrics.statuses.partial + metrics.statuses.fail;
      ...
      const realityIndex =
        principleAvg.INSPECTION_MANDATE || principleAvg.CONTINUOUS_VALIDATION || avgTrust / 10;
    ...
    const summary = {
      ...
      active: enhancedAgents.filter((a) => {
        const hrs = a.lastInteraction
          ? (Date.now() - new Date(a.lastInteraction).getTime()) / 3600000
          : 999;
        return hrs < 24;
      }).length,
      ...
      avgTrustScore:
        enhancedAgents.length > 0
          ? Math.round(
              enhancedAgents.reduce((sum, a) => sum + a.trustScore, 0) / enhancedAgents.length
            )
          : 85,
    };
 ...
 */
router.put(
  '/:id/external-systems/:systemName/toggle',
  protect,
  async (req: Request, res: Response): Promise<void> => {
    try {
      const agent = await Agent.findOne({
 ...
 */
router.post(
  '/:id/external-systems/:systemName/sync',
  protect,
  async (req: Request, res: Response): Promise<void> => {
    try {
