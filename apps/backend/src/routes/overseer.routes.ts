...
router.post(
  ...
  protect,
  requireTenant,
  requireScopes(['overseer:act']),
  async (req: Request, res: Response): Promise<void> => {
    try {
      const action = await BrainAction.findByIdAndUpdate(
        req.params.id,
        { status: 'approved', approvedBy: (req as any).userEmail || 'system' },
        { new: true }
      );
      if (!action) {
        res.status(404).json({ success: false, message: 'Not found' });
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const requireJustification = ['ban_agent', 'restrict_agent', 'quarantine_agent'].includes(
        action.type
      );
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let lastCycle = await BrainCycle.findOne().sort({ completedAt: -1 }).lean();

      if (!lastCycle) {
        // Edge case: try tenant-specific (shouldn't normally happen)
        lastCycle = await BrainCycle.findOne({ tenantId: tenant }).sort({ completedAt: -1 }).lean();
      }
      ...
      res.json({
        ...
        data: {
          ...
          message:
            lastCycle?.thought ||
            'Systems operating within normal parameters. Awaiting next analysis cycle.',
          metrics: lastCycle?.metrics
            ...
            : {
                agentCount: 0,
                systemTrustScore: 80.0, // Default 80/100 when no cycles
                alertsProcessed: 0,
                actionsPlanned: 0,
              },
