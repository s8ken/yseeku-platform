...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        errMsg.includes('AI_BILLING_ERROR') ||
        ...
        res.status(503).json({
          ...
          error:
            'The AI service is temporarily unavailable due to insufficient credits. Please contact your administrator.',
          code: 'BILLING_ERROR',
        });
        return;
      }
      ...
      if (
        errMsg.includes('AI_RATE_LIMIT_ERROR') ||
        ...
        res.status(429).json({
          ...
          error:
            'The AI service is currently rate limited. Please wait a moment before trying again.',
