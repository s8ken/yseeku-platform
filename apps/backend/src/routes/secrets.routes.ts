    ...
    try {
      ...
      if (!result) {
        ...
        res
          ...
          .json({
            success: true,
            message: 'Using env provider; store value in environment',
            data: { hint: `${name}_ENCRYPTED for KMS` },
          });
        return;
      }
      ...
      res
        .status(500)
        .json({ success: false, message: 'Failed to store secret', error: getErrorMessage(e) });
    }
  }
);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: 'Failed to read secret', error: getErrorMessage(e) });
    }
  }
);
