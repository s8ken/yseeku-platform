...
class ComplianceReportService {
    ...
    async generateReport(config) {
        ...
        try {
            ...
            logger_1.logger.error('Report generation failed', { error: (0, error_utils_1.getErrorMessage)(error), config });
            throw error;
        }
    }
    /**
     ...
     */
    async generateTrustSummary(config) {
        ...
        // Fetch alerts
        const alerts = await alert_model_1.AlertModel.find({
            tenantId,
        ...
        const agentSummaries = agents.map(agent => {
            const agentConvs = conversations.filter(c => c.agents.some((a) => a._id?.toString() === agent._id?.toString()));
     ...
     */
    async generateSymbiCompliance(config) {
        const { tenantId, startDate, endDate } = config;
     ...
     */
    async generateIncidentReport(config) {
        const { tenantId, startDate, endDate } = config;
        const alerts = await alert_model_1.AlertModel.find({
            tenantId,
            timestamp: { $gte: startDate, $lte: endDate },
     ...
     */
    async generateAgentAudit(config) {
     ...
     */
    async generateFullAudit(config) {
    ...
    // Helper methods
    createMeta(type, tenantId, startDate, endDate) {
        return {
    ...
    generatePrincipleDetail(score, principle) {
        return {
            score,
    ...
    generateRecommendations(avgTrust, complianceRate, alertCount) {
        const recs = [];
        if (avgTrust < 7) {
    ...
    generateDailyTimeline(dates, start, end) {
        const timeline = [];
...
<body>
  ...
  <footer style="margin-top: 40px; text-align: center; color: #666;">
    ...
    generateHtmlSummary(report, type) {
...
exports.complianceReportService = new ComplianceReportService();
