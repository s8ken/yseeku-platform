...
const metrics_2 = require("../../observability/metrics");
const uuid_1 = require("uuid");
const logger_1 = __importDefault(require("../../utils/logger"));
const memory_1 = require("./memory");
const constraints_1 = require("./constraints");
const error_utils_1 = require("../../utils/error-utils");
async function executeActions(tenantId, cycleId, actions, mode = 'advisory') {
    const results = [];
    for (const a of actions) {
        const status = mode === 'enforced' ? 'executed' : 'planned';
        const doc = await brain_action_model_1.BrainAction.create({
            cycleId,
            tenantId,
            type: a.type,
            target: a.target,
        ...
        if (mode === 'enforced') {
            ...
            if (!check.ok) {
                ...
                metrics_2.sonateRefusalsTotal.inc({ reason: String(check.rule || 'unknown'), tenant_id: tenantId });
                metrics_2.sonateRefusalLatencySeconds.observe({ reason: String(check.rule || 'unknown'), tenant_id: tenantId }, (Date.now() - startMs) / 1000);
            ...
            try {
                switch (a.type) {
                    ...
                    case 'unban_agent':
                        ...
                        break;
                    default:
                        logger_1.default.warn('Unknown action type', { type: a.type, target: a.target });
                ...
                doc.status = 'failed';
                doc.result = { error: (0, error_utils_1.getErrorMessage)(error) };
                await doc.save();
                logger_1.default.error('Action execution failed', {
                    type: a.type,
                    target: a.target,
                ...
                if (idx >= 0) {
                    results[idx].status = 'failed';
                    results[idx].result = { error: (0, error_utils_1.getErrorMessage)(error) };
                }
            }
        }
    }
 ...
 */
async function executeAlert(tenantId, action, doc) {
    await alerts_service_1.alertsService.create({
        type: 'system',
 ...
 */
async function executeAdjustThreshold(tenantId, action, doc) {
    const current = await settings_service_1.settingsService.getTrustThreshold(tenantId);
    const next = (current ?? 75) - 5;
    await settings_service_1.settingsService.setTrustThreshold(tenantId, next);
 ...
 */
async function executeBanAgent(tenantId, cycleId, action, doc) {
    // 1. Find the agent
    ...
    await agent.ban('system-brain', action.reason || 'System Brain automated ban', action.severity || 'medium', action.expiresAt);
    // 3. Clear agent sessions by archiving active conversations
    const archiveResult = await conversation_model_1.Conversation.updateMany({ agents: action.target, isArchived: false }, { $set: { isArchived: true } });
    ...
    await (0, memory_1.remember)(tenantId, 'action:ban_agent', {
    ...
    await doc.save();
    logger_1.default.info('Agent banned by System Brain', {
        agentId: action.target,
        agentName: agent.name,
        reason: action.reason,
        severity: action.severity,
 ...
 */
async function executeRestrictAgent(tenantId, cycleId, action, doc) {
    const agent = await agent_model_1.Agent.findById(action.target);
    if (!agent) {
        throw new Error(`Agent not found: ${action.target}`);
    }
    ...
    // Store in memory
    await (0, memory_1.remember)(tenantId, 'action:restrict_agent', {
        agentId: action.target,
        agentName: agent.name,
        reason: action.reason,
        restrictions,
    ...
    await doc.save();
    logger_1.default.info('Agent restricted by System Brain', {
        agentId: action.target,
        agentName: agent.name,
        restrictions,
    });
 ...
 */
async function executeQuarantineAgent(tenantId, cycleId, action, doc) {
    const agent = await agent_model_1.Agent.findById(action.target);
    if (!agent) {
        throw new Error(`Agent not found: ${action.target}`);
    }
    ...
    // Store in memory
    await (0, memory_1.remember)(tenantId, 'action:quarantine_agent', {
        agentId: action.target,
        agentName: agent.name,
        reason: action.reason,
        timestamp: new Date(),
    ...
    await doc.save();
    logger_1.default.info('Agent quarantined by System Brain', {
        agentId: action.target,
        agentName: agent.name,
        reason: action.reason,
    });
 ...
 */
async function executeUnbanAgent(tenantId, cycleId, action, doc) {
    const agent = await agent_model_1.Agent.findById(action.target);
    if (!agent) {
        throw new Error(`Agent not found: ${action.target}`);
    }
    ...
    // Store in memory
    await (0, memory_1.remember)(tenantId, 'action:unban_agent', {
        agentId: action.target,
        agentName: agent.name,
        previousStatus,
        reason: action.reason,
    ...
    await doc.save();
    logger_1.default.info('Agent unbanned by System Brain', {
        agentId: action.target,
        agentName: agent.name,
        previousStatus,
    });
