...
exports.liveMetricsService = void 0;
exports.initializeLiveMetrics = initializeLiveMetrics;
exports.recordMessage = recordMessage;
exports.recordError = recordError;
exports.emitTrustEvent = emitTrustEvent;
exports.emitAlert = emitAlert;
exports.emitUserMetrics = emitUserMetrics;
 ...
 */
function initializeLiveMetrics(socketServer) {
    io = socketServer;
    ...
    logger_1.logger.info('Live metrics service initialized');
}
/**
 * Record a message event for metrics
 */
function recordMessage(trustScore) {
 ...
 */
function emitUserMetrics(userId, metrics) {
    if (!io)
        return;
 ...
 */
async function calculatePrincipleScores() {
    const now = new Date();
    ...
    try {
        ...
        const receipts = await trust_receipt_model_1.TrustReceiptModel.find({
        ...
        logger_1.logger.warn('Failed to calculate principle scores from receipts, using baseline', {
...
async function broadcastMetrics() {
    ...
    try {
        ...
        logger_1.logger.error('Failed to broadcast metrics', { error: (0, error_utils_1.getErrorMessage)(error) });
    }
}
/**
...
async function calculateLiveMetrics() {
    ...
    const trustTrend = trustDelta > 0.5 ? 'up' : trustDelta < -0.5 ? 'down' : 'stable';
    // Fetch real-time counts from database
    const [activeAgents, activeConversations, alerts] = await Promise.all([
        agent_model_1.Agent.countDocuments({ lastActive: { $gte: oneHourAgo } }).catch(() => 0),
        conversation_model_1.Conversation.countDocuments({ lastActivity: { $gte: oneHourAgo } }).catch(() => 0),
    ...
    for (const a of alerts) {
        ...
        if (a._id === 'critical')
            alertCounts.critical = a.count;
        if (a._id === 'warning')
            alertCounts.warning = a.count;
    }
    ...
    const driftStatus = driftScore > 0.7 ? 'critical' : driftScore > 0.4 ? 'warning' : 'normal';
 ...
 */
async function getHistoricalMetrics(hours = 24, resolution = 'hour') {
    const now = new Date();
    ...
    try {
        ...
        const results = await conversation_model_1.Conversation.aggregate(pipeline);
        ...
        logger_1.logger.error('Failed to get historical metrics', { error: (0, error_utils_1.getErrorMessage)(error) });
