...
router.get('/kpis', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        const [recentConversations, previousConversations] = await Promise.all([
            conversation_model_1.Conversation.find({
                user: userId,
        ...
        const allConversations = await conversation_model_1.Conversation.find({ user: userId })
            .select('messages ethicalScore lastActivity createdAt');
        // Fetch agents
        const [activeAgents, allAgents] = await Promise.all([
            agent_model_1.Agent.find({
                user: userId,
                lastActive: { $gte: oneDayAgo },
            }).countDocuments(),
            agent_model_1.Agent.find({ user: userId }).countDocuments(),
        ]);
        // Calculate trust scores from conversations
        const calculateTrustMetrics = (conversations) => {
                ...
                for (const msg of conv.messages) {
                    ...
                    if (msg.metadata?.trustEvaluation?.trustScore?.principles) {
                        ...
                        if (principles.CONSENT_ARCHITECTURE)
                            principleScores.transparency += principles.CONSENT_ARCHITECTURE;
                        if (principles.INSPECTION_MANDATE)
                            principleScores.fairness += principles.INSPECTION_MANDATE;
                        if (principles.CONTINUOUS_VALIDATION)
                            principleScores.privacy += principles.CONTINUOUS_VALIDATION;
                        if (principles.ETHICAL_OVERRIDE)
                            principleScores.safety += principles.ETHICAL_OVERRIDE;
                        if (principles.RIGHT_TO_DISCONNECT)
                            principleScores.accountability += principles.RIGHT_TO_DISCONNECT;
                    }
                }
            }
            ...
            Object.keys(principleScores).forEach(key => {
                principleScores[key] =
                    principleScores[key] / messageCount || 8.5;
            });
            return {
        ...
        logger_1.default.info('KPI metrics calculated', {
            userId,
        ...
        logger_1.default.error('Get KPIs error', {
 ...
 */
router.get('/policy-status', auth_middleware_1.protect, async (req, res) => {
    try {
        const userId = req.userId;
        // Quick check on recent conversations for any critical violations
        const recentViolations = await conversation_model_1.Conversation.find({
            user: userId,
        ...
        logger_1.default.error('Get policy status error', { error: (0, error_utils_1.getErrorMessage)(error) });
 ...
 * - tenant?: string (currently unused, for future multi-tenant support)
 */
router.get('/risk', auth_middleware_1.protect, async (req, res) => {
    try {
        const userId = req.userId;
        // Fetch user's conversations for risk analysis
        const conversations = await conversation_model_1.Conversation.find({ user: userId })
            .select('messages ethicalScore lastActivity createdAt')
            .sort({ lastActivity: -1 });
        ...
        for (const conv of conversations) {
            for (const msg of conv.messages) {
                ...
                if (principles.ETHICAL_OVERRIDE)
                    principleScores.ethics += principles.ETHICAL_OVERRIDE;
                if (principles.RIGHT_TO_DISCONNECT)
                    principleScores.disconnect += principles.RIGHT_TO_DISCONNECT;
                if (principles.MORAL_RECOGNITION)
                    principleScores.moral += principles.MORAL_RECOGNITION;
        ...
        Object.keys(principleScores).forEach(key => {
            principleScores[key] =
                Math.round((principleScores[key] / messageCount) * 10);
        });
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayConvs = conversations.filter(conv => conv.lastActivity >= dayStart && conv.lastActivity <= dayEnd);
        ...
        logger_1.default.info('Risk metrics calculated', {
            userId,
            overallRiskScore,
            riskLevel,
            lowTrustMessages,
        ...
        logger_1.default.error('Get risk metrics error', {
            error: (0, error_utils_1.getErrorMessage)(error),
            userId: req.userId,
        });
        res.status(500).json({
