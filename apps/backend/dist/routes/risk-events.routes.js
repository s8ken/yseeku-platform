...
router.get('/', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        logger_1.default.info('Risk events fetched', {
            userId,
            total: summary.total,
        ...
        logger_1.default.error('Failed to query risk events', {
            error,
...
router.post('/:id/resolve', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        logger_1.default.info('Risk event marked as resolved', {
            userId,
            conversationId: id,
        });
        res.json({
        ...
        logger_1.default.error('Failed to get risk event by ID', {
            error,
            stack: (0, error_utils_1.getErrorStack)(error),
        });
        res.status(500).json({
