...
router.get('/', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        const conversations = await conversation_model_1.Conversation.find(query)
            .populate('agents', 'name model provider')
            .sort({ lastActivity: -1 })
            .limit(parseInt(limit))
            .skip(parseInt(offset));
        const total = await conversation_model_1.Conversation.countDocuments(query);
        res.json({
            success: true,
            data: {
                conversations,
...
router.put('/:id', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        if (contextTags !== undefined)
            conversation.contextTags = contextTags;
        if (ciEnabled !== undefined)
            conversation.ciEnabled = ciEnabled;
...
router.delete('/:id', auth_middleware_1.protect, async (req, res) => {
    try {
        const conversation = await conversation_model_1.Conversation.findOneAndDelete({
            _id: req.params.id,
            user: req.userId,
        });
        if (!conversation) {
 ...
 */
router.get('/:id/messages', auth_middleware_1.protect, async (req, res) => {
    try {
 ...
 */
router.post('/:id/messages', auth_middleware_1.protect, async (req, res) => {
    try {
        ...
        const consentWithdrawal = (0, detect_1.detectConsentWithdrawal)(content);
        // If consent withdrawal detected, handle it before generating AI response
        if (consentWithdrawal.detected) {
            logger_1.default.info('Consent withdrawal detected', {
            ...
            const withdrawalResponse = (0, detect_1.getWithdrawalResponse)(consentWithdrawal);
            const systemMessage = {
                sender: 'ai',
        ...
        if (generateResponse) {
            ...
            for (const agentId of conversation.agents) {
                const agentExists = await agent_model_1.Agent.findById(agentId);
                if (agentExists) {
            ...
            let agent = targetAgentId ? await agent_model_1.Agent.findById(targetAgentId) : null;
            if (!agent) {
                ...
                if (hasAnthropicKey || hasOpenAIKey) {
                    ...
                    agent = await agent_model_1.Agent.create({
                    ...
                    agent = await agent_model_1.Agent.findOne({ user: req.userId });
                    if (!agent) {
                        const publicAgent = await agent_model_1.Agent.findOne({ isPublic: true });
                        if (publicAgent) {
                            // Double-check the agent actually exists by trying to find it again
                            const verifiedAgent = await agent_model_1.Agent.findById(publicAgent._id);
                            if (verifiedAgent) {
                                agent = verifiedAgent;
                                targetAgentId = agent._id;
            ...
            try {
                ...
                const llmResponse = await llm_service_1.llmService.generate({
                ...
                try {
                    ...
                    const currentUser = await user_model_1.User.findById(req.userId);
                    ...
                    try {
                        await trust_receipt_model_1.TrustReceiptModel.updateOne({ self_hash: aiTrustEval.receiptHash }, {
                            $set: {
                                ...
                                previous_hash: aiTrustEval.receipt.previous_hash,
                        ...
                        logger_1.default.warn('Failed to persist AI trust receipt', { error: persistErr?.message || persistErr });
                    }
                    ...
                    if (aiTrustEval.status === 'FAIL' || aiTrustEval.status === 'PARTIAL') {
                        logger_1.default.warn('Trust violation in AI response', {
                            conversationId: conversation._id,
                            agentId: agent._id,
                            status: aiTrustEval.status,
                    ...
                    logger_1.default.error('Trust evaluation error for AI message:', trustError);
                ...
                logger_1.default.error('LLM generation error:', llmError);
        ...
        logger_1.default.error('Add message error:', error);
        res.status(500).json({
            success: false,
 ...
 * @access  Private
 */
router.post('/:id/export', auth_middleware_1.protect, async (req, res) => {
    try {
        const conversation = await conversation_model_1.Conversation.findOne({
            _id: req.params.id,
            user: req.userId,
        ...
        logger_1.default.error('Export conversation error:', error);
        res.status(500).json({
            success: false,
