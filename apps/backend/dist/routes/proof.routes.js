...
router.get('/verify/:id', async (req, res) => {
    try {
        ...
        const receipt = await trust_receipt_model_1.TrustReceiptModel.findOne({
            $or: [
                { _id: id },
                { self_hash: id },
                { session_id: id }
        ...
        logger_1.default.error('Failed to verify proof', { error: (0, error_utils_1.getErrorMessage)(error) });
        res.status(500).json({
            success: false,
            valid: false,
...
router.post('/verify', async (req, res) => {
    try {
        ...
        const existingReceipt = await trust_receipt_model_1.TrustReceiptModel.findOne({
            $or: [
