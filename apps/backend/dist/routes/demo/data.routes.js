 ...
 */
router.get('/overseer', async (req, res) => {
    try {
        const overseerData = {
            mode: 'advisory',
            status: 'active',
        ...
        logger_1.default.error('Demo overseer error', { error: (0, error_utils_1.getErrorMessage)(error) });
        res.status(500).json({
            success: false,
...
router.get('/interactions', async (req, res) => {
    try {
        ...
        if (searchQuery) {
            filtered = filtered.filter(i => i.summary.toLowerCase().includes(searchQuery) ||
        ...
        logger_1.default.error('Demo interactions error', { error: (0, error_utils_1.getErrorMessage)(error) });
        res.status(500).json({
            success: false,
...
router.get('/vls', async (req, res) => {
    try {
        ...
        logger_1.default.error('Demo VLS error', { error: (0, error_utils_1.getErrorMessage)(error) });
        res.status(500).json({
            success: false,
