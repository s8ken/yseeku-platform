...
let _canonicalVector: number[] | null = null;

async function getCanonicalVector(): Promise<number[]> {
  if (_canonicalVector) return _canonicalVector;
  
  // Create a semantic representation of the ideal scaffold
  const text = SCAFFOLD_KEYWORDS.join(' ');
...
async function alignmentEvidenceEnhanced(
  ...
  const chunkEmbeddings = await Promise.all(
    chunks.map(async (chunk) => {
      ...
      const similarity = stableCosineSimilarity(chunkEmbedding.vector, canonicalVector);
      return {
        ...chunk,
...
async function calculateRawResonance(transcript: Transcript): Promise<{ r_m: number; breakdown: any }> {
  const embedding = await embedder.embed(transcript.text);
