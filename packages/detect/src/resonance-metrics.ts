import {
  ...
  CalculationError,
  MathValidationError,
  calculateResonanceMetrics as coreCalculateResonanceMetrics,
} from '@sonate/core';
...
export function calculateResonanceMetrics(input: any): ResonanceMetrics {
  ...
  try {
    ...
    validated = {
      ...
      conversationHistory: parsed.conversationHistory?.map(
        (h: { role: 'user' | 'assistant'; content: string; timestamp?: Date }) => ({
          role: h.role,
          content: h.content,
  ...
  if (!isFinite(metrics.R_m) || metrics.R_m < 0 || metrics.R_m > 1) {
    throw new CalculationError('Invalid R_m result', 'calculateResonanceMetrics', {
      R_m: metrics.R_m,
      inputs: input,
    });
  }
