...
import { BedauIndexCalculator, SemanticIntent, SurfacePattern } from './bedau-index';
...
export class PerformanceBenchmarkingEngine {
  ...
  async benchmarkBedauCalculation(): Promise<BenchmarkResult> {
    ...
    for (let i = 0; i < iterations; i++) {
      const inputSize = inputSizes[i % inputSizes.length];
      const intent = this.createSemanticIntent(inputSize);
      const pattern = this.createSurfacePattern(inputSize);
    ...
    const result: BenchmarkResult = {
      ...
      passed,
      details: {
        input_size: Math.max(...inputSizes),
        iterations,
