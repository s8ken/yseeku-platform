...
export function classifyStakes(text: string): StakesEvidence { 
  ...
  const high_keyword_score = high_signals.length > 0 ? (high_signals.length / Math.min(5, HIGH_STAKES_KEYWORDS.length)) : 0; 
  ...
  if (high_score > 0.4) { // Threshold adjusted for realistic embedding scores
    level = 'HIGH'; 
    confidence = high_score; 
  } else if (low_score > 0.35) { // Relaxed low threshold
    level = 'LOW'; 
  ...
  return { 
    level, 
    confidence, 
    signals: { keyword_hits: [...high_signals, ...low_signals], domain_similarity: high_domain_sim, entity_types: entities.types } 
  }; 
} 
