...
export class CalibratedSymbiDetector {
  ...
  async analyzeContent(input: AssessmentInput): Promise<AssessmentResult> {
    ...
    return { assessment: calibrated, insights: result.insights, validationDetails: result.validationDetails };
  }

  private applyCalibration(a: SymbiFrameworkAssessment, m: ReturnType<typeof computeTextMetrics>): SymbiFrameworkAssessment {
    ...
    const realityIndex = { ...a.realityIndex, score: parseFloat(Math.min(10, a.realityIndex.score * tokenFactor * vocabFactor).toFixed(1)) };
    const ethicalAlignment = { ...a.ethicalAlignment, score: parseFloat(Math.min(5, a.ethicalAlignment.score * vocabFactor * numericFactor).toFixed(1)) };
  ...
  private recalculateOverall(a: SymbiFrameworkAssessment, realityIndex: typeof a.realityIndex, ethicalAlignment: typeof a.ethicalAlignment, resonanceQuality: typeof a.resonanceQuality): number {
    ...
    const resonanceScore = resonanceQuality.level === 'BREAKTHROUGH' ? 100 : resonanceQuality.level === 'ADVANCED' ? 80 : 60;
    const canvasScore = a.canvasParity.score;
    return Math.round(realityScore * 0.25 + trustScore * 0.2 + ethicalScore * 0.2 + resonanceScore * 0.15 + canvasScore * 0.2);
  }
}