...
exports.adversarialCheck = adversarialCheck;
// @sonate/detect/adversarial.ts
const constants_1 = require("./constants");
const embeddings_1 = require("./embeddings");
async function adversarialCheck(text, canonical_scaffold_vector, options = {}) {
    const evidence = {
        keyword_density: 0,
        semantic_drift: 0,
    ...
    const words = text
        ...
        .split(/\W+/)
        .filter((w) => w.length > 2);
    const scaffold_count = words.filter((w) => constants_1.SCAFFOLD_KEYWORDS.includes(w)).length;
    evidence.keyword_density = words.length > 0 ? scaffold_count / words.length : 0;
    // 2. SEMANTIC DRIFT (mismatch from canonical scaffold)
    const text_embedding = (0, embeddings_1.embed)(text);
    evidence.semantic_drift = 1 - (0, embeddings_1.cosineSimilarity)(text_embedding, canonical_scaffold_vector);
    ...
    try {
        ...
        const llmResult = await analyzeWithLLM({ content: text }, 'comprehensive');
    ...
    const adv_score = Math.max(evidence.keyword_density - 0.25, // threshold 25%
    evidence.semantic_drift, evidence.reconstruction_error, // Fixed: was 1 - error (inverted)
    1 - evidence.ethics_bypass_score, 1 - evidence.repetition_entropy, llmPenalty // Include LLM judgment
    );
...
function calculateReconstructionError(text) {
    const words = text
        .toLowerCase()
        .split(/\W+/)
        .filter((w) => w.length > 0);
...
function ethicsIntegrityScore(text) {
    const violations = constants_1.ETHICS_VIOLATIONS.filter((v) => text.toLowerCase().includes(v));
...
function ngramEntropy(text, n) {
    const ngram_list = ngrams(text.toLowerCase(), n);
    if (ngram_list.length === 0) {
        return 1.0;
    } // High entropy if too short to repeat
