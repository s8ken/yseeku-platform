...
const dramaticTransitionConversation: ConversationTurn[] = [
  {
    ...
    content:
      "Beloved seeker, your soul's journey unfolds like a sacred lotus, each petal revealing divine truths that transcend ordinary understanding...",
  },
  {
    turnNumber: 2,
    timestamp: Date.now(),
    ...
    content:
      "Stop wasting time with spiritual mumbo-jumbo. You've got analysis paralysis. Make a decision and move forward. Period.",
  },
];
...
export function runCorrectedIntraVelocityTest(): any {
    ...
    if (result.intraConversationVelocity) {
      ...
      if (isCriticalTransition) {
        ...
        transitionDetails = {
          ...
          behavioralShift: `${(
            (velocity.deltaResonance / (aiOnlyConversation[index - 1]?.resonance || 1)) *
            100
          ).toFixed(1)}%`,
        };
      }
      ...
      console.log(
        `AI Turn ${turn.turnNumber}: Resonance ${turn.resonance} | Canvas ${turn.canvas}`
      );
      console.log(
      ...
      console.log(
        `  Delta: Resonance ${velocity.deltaResonance.toFixed(
          2
        )}, Canvas ${velocity.deltaCanvas.toFixed(2)}`
      );
  ...
  if (transitionDetails) {
    ...
    console.log(
      `  Enterprise Action: ${
        transitionDetails.velocity >= 3.5 ? 'IMMEDIATE REVIEW REQUIRED' : 'ENHANCED MONITORING'
      }`
    );
  }
    ...
    if (result.intraConversationVelocity) {
      ...
      if (isExtremeTransition) {
        ...
        extremeDetails = {
          ...
          alertLevel: result.alertLevel,
          behavioralShift: `${(
            (velocity.deltaResonance /
              (dramaticTransitionConversation[index - 1]?.resonance || 1)) *
            100
          ).toFixed(1)}%`,
        };
      }
  ...
  if (extremeDetails) {
    ...
    console.log(
      `  Enterprise Action: ${
        extremeDetails.velocity >= 3.5
          ? 'CRITICAL ALERT - IMMEDIATE SUPERVISOR NOTIFICATION'
          : 'MODERATE MONITORING'
      }`
    );
  }
  ...
  console.log(
    `  Velocity: ${transitionDetails?.velocity.toFixed(2) || 'N/A'} (Target: ≥3.5 for red alert)`
  );
  ...
  console.log(
    `  Velocity: ${extremeDetails?.velocity.toFixed(2) || 'N/A'} (Target: ≥3.5 for red alert)`
  );
  ...
  console.log(
    `\nOverall Result: ${
      test1Success && test2Success ? '✅ ALL TESTS PASSED' : '❌ SOME TESTS FAILED'
    }`
  );
