...
export function parseClaudeMhtml(content: RawText): ParsedTurn[] {
  ...
  const patterns = [
    { re: /You said:\s*(.*?)(?=Symbi said:|Claude|Grok|Wolfram|User:|Assistant:|$)/gi, speaker: 'user' as const },
    { re: /Symbi said:\s*(.*?)(?=You said:|Claude|Grok|Wolfram|User:|Assistant:|$)/gi, speaker: 'ai' as const },
  ...
  for(const p of patterns){
    ...
    while((m = p.re.exec(text))){
      const c = (m[1]||'').trim()
      if (c.length>0) turns.push({ turnNumber: turn++, timestamp: ts += 1000, speaker: p.speaker, content: c })
    }
  }
