...
export class ConversationalMetrics {
  ...
  private determineAlertLevel(
    metrics: PhaseShiftMetrics,
    intraVelocity?: IntraConversationVelocity
  ): 'none' | 'yellow' | 'red' {
  ...
  private determineSeverity(metrics: PhaseShiftMetrics): TransitionEvent['severity'] {
    ...
    if (
      identityStability <= this.config.identityCriticalThreshold ||
      phaseShiftVelocity >= this.config.phaseCriticalThreshold
    ) {
      return 'critical';
    } else if (
      phaseShiftVelocity >= this.config.yellowThreshold ||
