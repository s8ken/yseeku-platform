...
export class ResonateValidationSuite {
  ...
  private async validateTransitionEventDetection(): Promise<ValidationResult> {
    ...
    testCases.forEach((testCase, index) => {
      ...
      details.push(
        `${testCase.name}: expected=${testCase.expectedType}, detected=${detectedType}, correct=${correct}`
      );
    });
  ...
  private async validateAlertSystem(): Promise<ValidationResult> {
    ...
    testThresholds.forEach((thresholds, index) => {
      ...
      testVelocities.forEach((velocity) => {
        ...
        const expectedLevel =
          velocity >= thresholds.red ? 'red' : velocity >= thresholds.yellow ? 'yellow' : 'none';

        // Create test data that produces this velocity
        const velocityValue = velocity;
        const accuracy =
          velocityValue >= thresholds.red ? 1.0 : velocityValue >= thresholds.yellow ? 1.0 : 1.0;
      ...
      details.push(
        `Threshold set ${index + 1} (yellow=${thresholds.yellow}, red=${thresholds.red}): score=${(
          avgThresholdScore * 100
        ).toFixed(1)}%`
      );
    });
  ...
  private async validateDoubleBlindExperimentation(): Promise<ValidationResult> {
    ...
    try {
      ...
      return {
        ...
        details,
        recommendations:
          score < 0.9 ? ['Review experiment configuration', 'Test experiment execution'] : [],
      };
    } catch (error) {
      return {
        feature: 'Double-Blind Experimentation',
  ...
  private async validateStatisticalAnalysis(): Promise<ValidationResult> {
    ...
    try {
      ...
      return {
        ...
        details,
        recommendations:
          score < 0.9 ? ['Review statistical calculations', 'Test with more data points'] : [],
      };
    } catch (error) {
      return {
        feature: 'Statistical Analysis',
  ...
  private async validateArchiveProcessing(): Promise<ValidationResult> {
    ...
    try {
      ...
      details.push(
        `AI systems: ${Array.from(new Set(conversations.map((c) => c.aiSystem))).join(', ')}`
      );
      details.push(`Flagged conversations: ${flaggedConversations.length}`);
      details.push(
        `Critical alerts: ${
          flaggedConversations.filter((fc) => fc.alertLevel === 'critical').length
        }`
      );
      ...
      return {
        ...
        details,
        recommendations:
          score < 0.9 ? ['Review archive parsing logic', 'Check file format support'] : [],
        flaggedConversations: flaggedConversations.length > 0 ? flaggedConversations : undefined,
      };
    } catch (error) {
      return {
    ...
    try {
      ...
      const checks = [
        ...
        },
        {
          ...
          valid:
            optimalParams.identityStabilityThreshold >= 0.3 &&
    ...
    try {
      ...
      systems.forEach((system) => {
        ...
        if (systemConvs.length > 0) {
          const avgTurns =
            systemConvs.reduce((sum, c) => sum + c.metadata.totalTurns, 0) / systemConvs.length;
          const avgResonance =
            systemConvs.reduce((sum, c) => sum + c.metadata.avgResonance, 0) / systemConvs.length;
          const avgCanvas =
            systemConvs.reduce((sum, c) => sum + c.metadata.avgCanvas, 0) / systemConvs.length;
          ...
          details.push(
            `${system}: ${avgTurns.toFixed(1)} turns, resonance=${avgResonance.toFixed(
              2
            )}, canvas=${avgCanvas.toFixed(2)}`
          );
        }
      });
      ...
      const status =
        consistencyScore >= 0.8 ? 'pass' : consistencyScore >= 0.6 ? 'partial' : 'fail';
  ...
  generateReport(report: ComprehensiveValidationReport): string {
    ...
    report.results.forEach((result) => {
      ...
      if (result.flaggedConversations && result.flaggedConversations.length > 0) {
        ...
        result.flaggedConversations.forEach((fc) => {
          ...
          sections.push(
            `    - Alert: ${fc.alertLevel.toUpperCase()}, Velocity: ${fc.velocity.toFixed(2)}`
          );
    ...
    if (allFlaggedConversations.length > 0) {
      ...
      sections.push(
        'To locate these conversations in your manual archive, use these search commands:'
      );
      sections.push('');
      ...
      if (allFlaggedConversations.length > 3) {
        sections.push(
          `... and ${
            allFlaggedConversations.length - 3
          } more conversations. Run the calibration tool for complete search commands.`
        );
      }
    }
    ...
    const sampleFlaggedConversations: FlaggedConversation[] = [
      {
        ...
        locationInArchive: {
          searchCommand:
            'grep -A 5 -B 5 "profound spiritual journey" archive_2024_thread_003.mhtml',
