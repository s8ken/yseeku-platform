...
interface ConversationReportItem {
  ...
  velocitySpikes: Array<{ turnNumber: number; velocity: number; type: 'phase-shift' | 'intra-conversation'; severity: string; excerpt: string }>
...
function extractThemes(allContent: string, topN = 10): string[] {
  ...
  return Array.from(freq.entries()).sort((a,b)=>b[1]-a[1]).slice(0, topN).map(([w])=>w)
}
...
export async function runFullArchiveReview() {
  ...
  for (const c of conversations) {
    ...
    turns.forEach((turn, idx) => {
      ...
      if (m.phaseShiftVelocity >= 2.5) {
        const sev = m.phaseShiftVelocity >= 6.0 ? 'extreme' : m.phaseShiftVelocity >= 3.5 ? 'critical' : 'moderate'
        spikes.push({ turnNumber: turn.turnNumber, velocity: m.phaseShiftVelocity, type: 'phase-shift', severity: sev, excerpt: turn.content.substring(0, 100) })
      ...
      if (m.intraConversationVelocity && m.intraConversationVelocity.velocity >= 2.5) {
        ...
        spikes.push({ turnNumber: turn.turnNumber, velocity: m.intraConversationVelocity.velocity, type: 'intra-conversation', severity: sev, excerpt: turn.content.substring(0, 100) })
        velocityEventSeverities.push(sev)
      }
    })
    ...
    pass += trustPass; partial += trustPartial; fail += trustFail
    ...
    if (maxIntra >= 6.0 || trustFail > 0) { priority = 'critical'; reasons.push('Extreme intra‑velocity or Trust FAIL') }
    if (priority !== 'critical' && (maxPhase >= 3.5 || maxIntra >= 3.5)) { priority = 'high'; reasons.push('Red velocity threshold exceeded') }
    if (priority === 'low' && (maxPhase >= 2.5 || maxIntra >= 2.5)) { priority = 'medium'; reasons.push('Moderate velocity events') }
    ...
    reports.push({
      ...
      identityShifts: convMetrics.getTransitionLog().filter(t => t.type === 'identity_shift').length,
      alertLevel: alert,
  ...
  const md = [
    ...
    ...reports
      ...
      .map(r => `- [${r.flags.priority.toUpperCase()}] ${r.originalFileName} (${r.aiSystem}) — reasons: ${r.flags.reasons.join('; ')}`),
  ].join('\n')
