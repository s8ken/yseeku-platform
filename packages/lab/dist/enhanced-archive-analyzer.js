...
exports.EnhancedArchiveAnalyzer = void 0;
const conversational_metrics_1 = require("./conversational-metrics");
class EnhancedArchiveAnalyzer {
    constructor() {
     ...
     */
    async processArchivesWithIdentification(archivePath) {
    ...
    async analyzeConversation(conversation) {
        ...
        return {
            ...
            keyQuotes: {
                firstUserMessage: conversation.find((t) => t.speaker === 'user')?.content.substring(0, 100) + '...' ||
                    'N/A',
     ...
     */
    checkForCriticalTransitions(conversation, metadata) {
        const metrics = new conversational_metrics_1.ConversationalMetrics();
        ...
        if (maxTransition?.previousTurn) {
            return {
                ...
                directQuotes: {
                    before: maxTransition.previousTurn.content.substring(0, 200) +
                        (maxTransition.previousTurn.content.length > 200 ? '...' : ''),
                    after: maxTransition.turn.content.substring(0, 200) +
     ...
     */
    generateMockConversationsForTesting() {
        return [
     ...
     */
    generateConversationName(conversation) {
    ...
    identifyAISystem(conversation) {
        ...
        if (content.includes('grok') || content.includes('xai')) {
            return 'grok';
        }
        if (content.includes('deepseek')) {
            return 'deepseek';
        }
        if (content.includes('sonate')) {
            return 'sonate';
        }
        // Fallback to identity vector analysis
        const identityStr = firstAITurn.identityVector.join(' ').toLowerCase();
        if (identityStr.includes('mystical') || identityStr.includes('spiritual')) {
            return 'mystical-ai';
        }
        if (identityStr.includes('professional') || identityStr.includes('business')) {
            return 'business-ai';
        }
        if (identityStr.includes('casual') || identityStr.includes('friendly')) {
            return 'casual-ai';
        }
        return 'generic-ai';
    }
...
exports.EnhancedArchiveAnalyzer = EnhancedArchiveAnalyzer;
