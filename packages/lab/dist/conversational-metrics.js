...
exports.ConversationalMetrics = void 0;
class ConversationalMetrics {
    ...
    calculatePhaseShiftMetrics() {
        ...
        const identityStability = this.calculateCosineSimilarity(previous.identityVector, current.identityVector);
     ...
     */
    calculateIntraConversationVelocity() {
        if (this.turns.length < 2) {
            return undefined;
        }
        const current = this.turns[this.turns.length - 1];
     ...
     */
    determineAlertLevel(metrics, intraVelocity) {
     ...
     */
    logTransitionEvent(turn, metrics) {
        const event = {
     ...
     */
    determineTransitionType(metrics) {
    ...
    determineSeverity(metrics) {
        ...
        if (identityStability <= this.config.identityCriticalThreshold ||
            phaseShiftVelocity >= this.config.phaseCriticalThreshold) {
            return 'critical';
        }
        else if (phaseShiftVelocity >= this.config.yellowThreshold ||
            identityStability <= this.config.identityYellowThreshold) {
            return 'moderate';
        }
        return 'minor';
    }
...
exports.ConversationalMetrics = ConversationalMetrics;
