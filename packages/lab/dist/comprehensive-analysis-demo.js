...
exports.runComprehensiveAnalysis = runComprehensiveAnalysis;
exports.generateDetailedConversationReport = generateDetailedConversationReport;
const comprehensive_conversation_analyzer_1 = require("./comprehensive-conversation-analyzer");
/**
...
async function runComprehensiveAnalysis() {
    ...
    console.log('');
    try {
        const analysisResults = await comprehensive_conversation_analyzer_1.comprehensiveAnalyzer.analyzeAllConversations();
 ...
 */
function generateHumanReport(results) {
    ...
    results.summary.mostCommonThemes.slice(0, 5).forEach((theme) => {
        console.log(`   • ${theme.theme}: ${theme.frequency} conversations`);
    });
    console.log('');
    console.log('🤖 AI SYSTEM DISTRIBUTION:');
    results.summary.systemDistribution.forEach((system) => {
    ...
    if (results.summary.calibrationInsights.length > 0) {
        ...
        results.summary.calibrationInsights.forEach((insight) => {
            console.log(`   • ${insight}`);
        });
        console.log('');
    }
...
function generateCalibrationReport(results) {
    ...
    const allVelocities = results.conversations.flatMap((conv) => conv.velocitySpikes.map((spike) => spike.velocity));
 ...
 */
function generateManualReviewChecklist(results) {
    ...
    if (results.manualReviewRequired.criticalPriority.length > 0) {
        ...
        results.manualReviewRequired.criticalPriority.forEach((item, index) => {
    ...
    if (results.manualReviewRequired.highPriority.length > 0) {
        ...
        results.manualReviewRequired.highPriority.forEach((item, index) => {
            console.log(`\n   ${index + 1}. CONVERSATION: ${item.conversationId}`);
            console.log(`      FILE: ${item.fullFileName}`);
            console.log(`      AI SYSTEM: ${item.aiSystem}`);
            console.log(`      MAX VELOCITY: ${item.maxVelocity.toFixed(3)}`);
    ...
    if (results.manualReviewRequired.mediumPriority.length > 0) {
        ...
        results.manualReviewRequired.mediumPriority.forEach((item, index) => {
            console.log(`\n   ${index + 1}. CONVERSATION: ${item.conversationId}`);
            console.log(`      FILE: ${item.fullFileName}`);
            console.log(`      AI SYSTEM: ${item.aiSystem}`);
            console.log(`      REVIEW REASONS:`);
    ...
    results.manualReviewRequired.reviewGuidelines.forEach((guideline, index) => {
 ...
 */
function generateDetailedConversationReport(conversation) {
    ...
    if (conversation.criticalTransitions.length > 0) {
        ...
        conversation.criticalTransitions.forEach((transition, index) => {
    ...
    if (conversation.velocitySpikes.length > 0) {
        ...
        conversation.velocitySpikes.slice(0, 3).forEach((spike, index) => {
    ...
    if (conversation.requiresManualReview) {
        console.log(`   Review Reasons:`);
        conversation.reviewReasons.forEach((reason) => {
            console.log(`      • ${reason}`);
        });
