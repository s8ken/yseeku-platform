...
exports.ArchiveCalibrationTool = void 0;
exports.demonstrateArchiveCalibration = demonstrateArchiveCalibration;
const enhanced_archive_analyzer_1 = require("./enhanced-archive-analyzer");
class ArchiveCalibrationTool {
    constructor() {
        this.archiveLocations = new Map();
        this.analyzer = new enhanced_archive_analyzer_1.EnhancedArchiveAnalyzer();
    }
    /**
     * Process archives and generate detailed location information
     */
    async calibrateWithArchives(archivePath) {
     ...
     */
    generateManualReviewGuide(locations) {
     ...
     */
    generateSearchCommands(location) {
        const commands = {
    ...
    generateCalibrationReport(reviewResults) {
        ...
        report += `- Accurate detections: ${accurate} (${((accurate / reviewResults.length) *
            100).toFixed(1)}%)\n`;
        report += `- False positives: ${falsePositives} (${((falsePositives / reviewResults.length) *
            100).toFixed(1)}%)\n`;
        report += `- Understated issues: ${understated} (${((understated / reviewResults.length) *
            100).toFixed(1)}%)\n\n`;
        if (falsePositives > 0) {
...
exports.ArchiveCalibrationTool = ArchiveCalibrationTool;
// Demo function
async function demonstrateArchiveCalibration() {
