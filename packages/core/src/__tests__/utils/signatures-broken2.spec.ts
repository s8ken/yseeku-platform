...
jest.mock('../../utils/signatures', () => {
  ...
  return {
    ...
    verifySignature: jest
      .fn()
      .mockImplementation(async (signature: string, payload: string, publicKey: Uint8Array) => {
...
describe('Mock Signature Utilities', () => {
  ...
  describe('signPayload', () => {
    it('should sign a payload', async () => {
      ...
      expect(signature).toBe(
        'mock-signature-' + testPayload.length + '-' + testKeyPair.privateKey.length
      );
    });
    ...
    it('should handle Unicode payloads', async () => {
      ...
      expect(signature).toBe(
        'mock-signature-' + unicodePayload.length + '-' + testKeyPair.privateKey.length
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle very large payloads', async () => {
      ...
      expect(signature).toBe(
        'mock-signature-' + largePayload.length + '-' + testKeyPair.privateKey.length
      );
    });
    ...
    it('should handle special characters', async () => {
      ...
      expect(signature).toBe(
        'mock-signature-' + specialPayload.length + '-' + testKeyPair.privateKey.length
      );
    });
