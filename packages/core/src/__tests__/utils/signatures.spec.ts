...
describe('Cryptographic Signature Utilities', () => {
  ...
  describe('signPayload', () => {
    ...
    it('should reject invalid private keys', async () => {
      ...
      await expect(signPayload(testPayload, invalidKey))
        .resolves.toBeDefined(); // Ed25519 can handle all-zero keys
    });

    it('should reject wrong length private keys', async () => {
      const shortKey = new Uint8Array(16);
      await expect(signPayload(testPayload, shortKey))
        .rejects.toThrow();
    });
  });
  ...
  describe('verifySignature', () => {
    ...
    it('should reject signature for different payload', async () => {
      const isValid = await verifySignature(validSignature, 'different-payload', testKeyPair.publicKey);
    ...
    it('should handle Unicode payload verification', async () => {
      ...
      const isValid = await verifySignature(unicodeSignature, unicodePayload, testKeyPair.publicKey);
    ...
    it('should reject malformed public keys', async () => {
      ...
      for (const malformedKey of malformedKeys) {
        await expect(verifySignature(validSignature, testPayload, malformedKey))
          .resolves.toBe(false); // Should return false, not throw
      }
    });
  });
  ...
  describe('Security Tests', () => {
    ...
    it('should generate cryptographically secure keys', async () => {
      const keyPairs = await Promise.all(
        Array(100).fill(null).map(() => generateKeyPair())
      );
      ...
      for (const char of hexChars) {
        const count = (privateKeyBytes.toString('hex') + publicKeyBytes.toString('hex')).split(char).length - 1;
