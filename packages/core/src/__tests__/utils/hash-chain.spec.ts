...
import { hashChain, verifyHashChain, genesisHash } from '../../utils/hash-chain';
...
describe('Hash Chain Utilities', () => {
  ...
  describe('Performance Tests', () => {
    ...
    it('should handle concurrent hash generation', async () => {
      ...
      for (let i = 0; i < concurrentOperations; i++) {
        promises.push(
          Promise.resolve(
            hashChain(`hash-${i}`, `payload-${i}`, testTimestamp + i, `signature-${i}`)
          )
        );
      }
  ...
  describe('Security Tests', () => {
    ...
    it('should have good hash distribution', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const hash = hashChain(`hash-${i}`, `payload-${i}`, testTimestamp + i, `signature-${i}`);
        hashes.push(hash);
      }
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle numeric inputs converted to strings', () => {
      const hash = hashChain(123 as any, 456 as any, 789 as any, 0o12 as any);
