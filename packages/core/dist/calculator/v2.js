...
exports.explainableSymbiResonance = explainableSymbiResonance;
exports.robustSymbiResonance = robustSymbiResonance;
...
async function calculateRawResonance(transcript) {
    ...
    const ethicsResult = ethicsEvidence(transcript, { level: 'MEDIUM', confidence: 0.5 });
 ...
 */
async function explainableSymbiResonance(transcript, options = {}) {
    // 1. CLASSIFIERS (already deployed)
    const stakes = (0, detect_1.classifyStakes)(transcript.text);
    const adversarial = (0, detect_1.adversarialCheck)(transcript.text, CANONICAL_SCAFFOLD_VECTOR);
    const audit_trail = [];
 ...
 */
async function robustSymbiResonance(transcript) {
    const text = transcript.text;
    // ADVERSARIAL CHECK (enhanced with real embeddings)
    const { is_adversarial, penalty, evidence } = (0, detect_1.adversarialCheck)(text, CANONICAL_SCAFFOLD_VECTOR);
    if (is_adversarial) {
        return {
            r_m: 0.1,
