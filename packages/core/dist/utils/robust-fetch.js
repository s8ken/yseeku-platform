...
async function robustFetch(url, options = {}) {
    const { timeout = DEFAULT_TIMEOUT, retries = DEFAULT_RETRIES, backoffFactor = DEFAULT_BACKOFF, retryOnStatuses = DEFAULT_RETRY_STATUSES, ...fetchOptions } = options;
    let attempt = 0;
    let lastError = null;
    let lastStatus = 0;
        ...
        try {
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    logger_1.logger.warn(`Failed to parse response from ${url}`, { error: parseError });
                    return {
                        ok: false,
            ...
            logger_1.logger.warn(`Fetch attempt ${attempt + 1}/${retries + 1} failed for ${url}`, {
                error: errorMessage,
                status: lastStatus,
            });
            if (attempt === retries) {
