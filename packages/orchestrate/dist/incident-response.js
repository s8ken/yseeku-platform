...
exports.incidentResponseSystem = exports.IncidentResponseSystem = void 0;
exports.createIncidentResponseSystem = createIncidentResponseSystem;
/**
 * Automated Incident Response System
 */
class IncidentResponseSystem {
     ...
     */
    async checkForIncidents(context) {
     ...
     */
    async createIncident(rule, context) {
     ...
     */
    async executeAlert(incident, config) {
        ...
        if (config.channels?.includes('email')) {
            console.log(`[EMAIL] Would send to: ${this.escalationPolicies
        ...
        if (config.channels?.includes('sms')) {
            console.log(`[SMS] Would send to: ${this.escalationPolicies
                .get(incident.severity)?.[0]
                .notify.join(', ')}`);
        }
    }
     ...
     */
    async executeBlock(incident, config) {
     ...
     */
    async executeThrottle(incident, config) {
     ...
     */
    async executeRestart(incident, config) {
     ...
     */
    async executeScale(incident, config) {
     ...
     */
    getOpenIncidents() {
        return Array.from(this.incidents.values()).filter((inc) => inc.status !== 'resolved' && inc.status !== 'closed');
    }
    /**
    ...
    getStatistics() {
        ...
        return {
            ...
            openIncidents: allIncidents.filter((inc) => inc.status !== 'resolved' && inc.status !== 'closed').length,
        };
    }
    /**
     * Get metric history for anomaly detection
     */
    getMetricHistory(metric, count) {
     ...
     * Clear old incidents
     */
    clearOldIncidents(maxAge = 7 * 24 * 60 * 60 * 1000) {
...
exports.IncidentResponseSystem = IncidentResponseSystem;
/**
 * Create incident response system instance
 */
function createIncidentResponseSystem() {
    return new IncidentResponseSystem();
}
/**
 * Global instance
 */
exports.incidentResponseSystem = createIncidentResponseSystem();
