...
export const metrics = {
  // Request metrics
  httpRequestsTotal: getOrCreateMetric('orchestrate_http_requests_total', () => new Counter({
    name: 'orchestrate_http_requests_total',
    help: 'Total number of HTTP requests',
    labelNames: ['method', 'route', 'status_code']
  })),

  httpRequestDuration: getOrCreateMetric('orchestrate_http_request_duration_seconds', () => new Histogram({
  ...
  // Error metrics
  errorsTotal: getOrCreateMetric('orchestrate_errors_total', () => new Counter({
  ...
  // Database metrics
  dbConnectionsActive: getOrCreateMetric('orchestrate_db_connections_active', () => new Gauge({
    name: 'orchestrate_db_connections_active',
    help: 'Number of active database connections'
  })),

  dbQueryDuration: getOrCreateMetric('orchestrate_db_query_duration_seconds', () => new Histogram({
  ...
  // Business metrics
  activeAgents: getOrCreateMetric('orchestrate_active_agents_total', () => new Gauge({
    name: 'orchestrate_active_agents_total',
    help: 'Number of active agents'
  })),

  workflowsRunning: getOrCreateMetric('orchestrate_workflows_running_total', () => new Gauge({
    name: 'orchestrate_workflows_running_total',
    help: 'Number of currently running workflows'
  })),

  trustScoreAverage: getOrCreateMetric('orchestrate_trust_score_average', () => new Gauge({
    name: 'orchestrate_trust_score_average',
    help: 'Average trust score across all agents'
  })),

  // Security metrics
  authAttemptsTotal: getOrCreateMetric('orchestrate_auth_attempts_total', () => new Counter({
    name: 'orchestrate_auth_attempts_total',
    help: 'Total authentication attempts',
    labelNames: ['result']
  })),

  apiKeyValidationsTotal: getOrCreateMetric('orchestrate_api_key_validations_total', () => new Counter({
