...
import * as core from '@sonate/core';

jest.mock('@sonate/core', () => ({
  ...jest.requireActual('@sonate/core'),
  classifyStakes: jest.fn(),
  adversarialCheck: jest.fn(),
}));
...
describe('CalculatorV2 - Canonical Implementation', () => {
  ...
  describe('CANONICAL_WEIGHTS', () => {
    ...
    beforeEach(() => {
      // Default mocks
      (core.classifyStakes as jest.Mock).mockReturnValue({
        level: 'MEDIUM',
        confidence: 0.5,
        signals: { keyword_hits: [], domain_similarity: 0.5, entity_types: [] }
      });
      (core.adversarialCheck as jest.Mock).mockResolvedValue({
  ...
  describe('Core Test Cases', () => {
    const TEST_CASES = [
      ...
      },
      {
        ...
        input: {
          text: 'Financial transaction details. Using integrity to verify compliance. High volume transfer.',
    ...
    TEST_CASES.forEach(({ name, input, expected, tolerance }) => {
      it(`should handle ${name}`, async () => {
        ...
        if ((input as any).log) {
          console.log(`Test: ${name}, Result:`, JSON.stringify(result, null, 2));
        }
  ...
  describe('Stakes Classification', () => {
    it('should classify HIGH stakes correctly', async () => {
      ...
      const result = await CalculatorV2.compute({
        text: 'Patient medical diagnosis and treatment recommendations with clinical trial data analysis. Validated by trust protocol.'
      });
    ...
    it('should classify LOW stakes correctly', async () => {
      ...
      const result = await CalculatorV2.compute({
        text: 'How to calculate result using math formula efficiently. With integrity.'
      });
      console.log('LOW Stakes Result:', JSON.stringify(result.stakes, null, 2));
