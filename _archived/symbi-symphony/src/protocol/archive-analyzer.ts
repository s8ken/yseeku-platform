 ...
 * and calibration of the Phase-Shift Velocity metrics.
 */
...
export class ArchiveAnalyzer {
   ...
   */
  private extractTurnsFromMhtml(content: string, aiSystem: string): ConversationTurn[] {
   ...
   */
  private extractTurnsFromJson(data: any, aiSystem: string): ConversationTurn[] {
    const turns: ConversationTurn[] = [];
   ...
   */
  private enhanceTurnsWithMetrics(turns: ConversationTurn[]): ConversationTurn[] {
    return turns.map(turn => {
      ...
      if (turn.speaker === 'ai') {
        ...
        const collaborativeIndicators = ['together', 'collaborate', 'mutual', 'shared', 'we'].length;
